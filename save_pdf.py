#code to store pdf values across realizations in text files for both dfn and graph
#this data is used to plot the mean with max and min
import numpy as np
import sys
import math
...
for realization_number in range(1,101):
    ...
    flag=True #flag for histogram function: True for PDF, False for actual distribution of number of particles
    nbins=10
    
    bx1, pdf1, cdf1 = bin_and_create_pdf(t1, nbins, flag, True) #graph
    bx2, pdf2, cdf2 = bin_and_create_pdf(t2, nbins, flag, False) #dfn
    
    np.savetxt('graph%s.txt' %realization_number, zip(bx1, pdf1), fmt="%10.7e %10.7e")
    np.savetxt('dfn%s.txt' %realization_number, zip(bx2, pdf2), fmt="%10.7e %10.7e")
