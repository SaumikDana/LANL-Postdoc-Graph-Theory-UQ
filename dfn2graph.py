...
"""
    ...
    elif graph_type == "intersection": #saumik added well_problem flag
        G = create_intersection_graph(inflow, outflow, well_problem)
...
def create_intersection_graph(inflow, outflow, well_problem, intersection_file="intersection_list.dat"):
    ...
    inflow_index=list() #saumik  
    outflow_index=list() #saumik
    
    for i in range(len(inflow)):
        inflow_index.append(boundary_index(inflow[i])) #saumik

    for i in range(len(outflow)):
        outflow_index.append(boundary_index(outflow[i])) #saumik
    ...
    # each fracture intersection in the DFN is a node in the graph
    for i in range(len(frac_edges)):
        f1 = int(frac_edges[i][0]) - 1
        #saumik
        if well_problem and f1 == 0:
           f1 = 't' #treating fracture #1 as sink
        keep = True
        if int(frac_edges[i][1]) > 0:
           f2 = int(frac_edges[i][1]) - 1
           if well_problem and f2 == 0:
              f2 = 't' #treating fracture #1 as sink
        elif check(int(frac_edges[i][1]),inflow_index):
           f2 = 's'
        elif check(int(frac_edges[i][1]),outflow_index):
           f2 = 't'
        elif int(frac_edges[i][1]) < 0:
           keep = False #no flow boundaries
    ...
    # list out the line numbers in intersection_file.dat that represent
    # fracture intersections (first two columns being positive numbers)
    # or fractures intersecting flow boundary condition faces
        ...
        for j in nodes:
            if i != j:
                ...
                if len(x) > 0:
                    ...
                    # This stops boundary fractures from being incorrectly connected 
